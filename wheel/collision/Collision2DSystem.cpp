...
#include "BoxBoxCollision2D.h"
#include "CircleBoxCollision2D.h"
#include "CircleCircleCollision2D.h"
#include "core/Scene.h"


void Wheel::Collision::Collision2DSystem::Update(float deltaTime)
{
    CheckNarrowPhase();
}

void Wheel::Collision::Collision2DSystem::SetDirtyFlag(Core::Description boxComponentDesc, Core::Description circleComponentDesc, bool enable)
{
...
void Wheel::Collision::Collision2DSystem::CheckNarrowPhase()
{
    m_Manifolds.clear();
    if (!m_BoxColliderPool || !m_TransformPool || !m_CircleColliderPool || !m_DescriptionRef) {
        m_BoxColliderPool  = m_Scene->GetComponentPool<BoxCollider2D>();
        m_CircleColliderPool  = m_Scene->GetComponentPool<CircleCollider2D>();
    ...
    {
        Core::Description desc1 = m_DescriptionRef->at(m_EntityIDs[i]);
        ...
        {
            Collision2DManifold manifold = Collision2DManifold();
            Core::Description desc2 = m_DescriptionRef->at(m_EntityIDs[j]);
            ...
            if (desc1HasBoxComponent && desc2HasBoxComponent)
            {
                ...
                manifold = BoxBoxCollision2D::BoxBoxCollision(collider1, transform1, collider2, transform2);
            }
            else if (desc1HasBoxComponent && desc2HasCircleComponent)
            {
                ...
                BoxBoxCollision2D::GetVertices(boxCollider, transform1); // ensure cachedVertices are current
                manifold = CircleBoxCollision2D::CheckCircleBoxCollision(transform2, circleCollider, transform1, boxCollider);
            }
            else if (desc2HasBoxComponent && desc1HasCircleComponent)
            {
                ...
                BoxBoxCollision2D::GetVertices(boxCollider, transform2); // ensure cachedVertices are current
                manifold = CircleBoxCollision2D::CheckCircleBoxCollision(transform1, circleCollider, transform2, boxCollider);
            }
            else if (desc1.HasComponentType(circleComponentDesc) && desc2.HasComponentType(circleComponentDesc))
            {
                ...
                manifold = CircleCircleCollision2D::CheckCircleCircleCollision(transform1, transform2, collider1, collider2);
            }
